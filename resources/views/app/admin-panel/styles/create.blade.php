...
@section('breadcrumbs')
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0">Create Style</h2>
                <div class="breadcrumb-wrapper">
                    {{ Breadcrumbs::render('style.create') }}
                </div>
            </div>
        </div>
    </div>
...
@section('content')
    <div class="card">
        <form action="{{route('style.store')}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body">
                @csrf
                {{ view('app.admin-panel.styles.form-fields') }}
            </div>
...
<script>
    ...
    $(document).on('submit','#style_form',function(e){
        e.preventDefault();
        let data = new FormData(this);
        $.ajax({
            type: "post",
            url: "{{route('style.store')}}",
            data: data,
            dataType: "json",
            contentType: false,
            processData: false,
            ...
            success: function (response) {
                ...
                {
                    toastr.success('Style Added Successfully.');

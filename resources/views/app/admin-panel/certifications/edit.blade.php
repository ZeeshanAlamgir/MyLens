...
@section('page-title', 'Update Certificate')
...
@section('breadcrumbs')
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0">Update Certificate</h2>
                <div class="breadcrumb-wrapper">
                    {{ Breadcrumbs::render('certificate.edit', $certificate['id']) }}
                </div>
            </div>
        </div>
    </div>
...
@section('content')
    <div class="card">
        <form id="certificate_update_form" enctype="multipart/form-data">
            ...
            <div class="card-body">
                @csrf
                {{ view('app.admin-panel.certifications.form-fields',['certificate'=>$certificate]) }}
            </div>
...
<script>
    ...
    $(document).on('submit','#certificate_update_form',function(e){
        e.preventDefault();
        let data = new FormData(this);
        let id = {{$certificate->id}};
        $.ajax({
            type: "post",
            url: "{{url('admin/certifications/update')}}/"+id,
            data: data,
            dataType: "json",
            contentType: false,
            processData: false,
    ...
    FilePond.create(document.getElementById('image'), {
        ...
        files: [
            {
                source: "{{ asset('app-assets/images/certification/' . $certificate->image) }}",
            }
        ],

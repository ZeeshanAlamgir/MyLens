...
@section('breadcrumbs')
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0">Create Actor Product</h2>
                <div class="breadcrumb-wrapper">
                    {{ Breadcrumbs::render('actor-product.create') }}
                </div>
            </div>
        </div>
    </div>
...
@section('content')
    <div class="card">
        <form action="{{route('actor-product.store')}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body">
                @csrf
                {{ view('app.admin-panel.actor-product.form-fields',compact('products')) }}
...
<script>
    ...
    $(document).on('submit','#actor_product__form',function(e){
        e.preventDefault();
        let data = new FormData(this);
        $.ajax({
            type: "post",
            url: "{{route('actor-product.store')}}",
            data: data,
            dataType: "json",
            contentType: false,
            processData: false,
            ...
            success: function (response) 
            {
                ...
                {
                    toastr.success('Actor Product Added Successfully.');

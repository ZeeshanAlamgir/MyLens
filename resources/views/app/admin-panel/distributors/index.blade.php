...
@section('breadcrumbs')
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0">Distributor</h2>
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form id="distributors-datatable" action="{{route('distributor.destroy')}}" method="get">
                {{ $dataTable->table() }}
            </form>
        </div>
    </div>
...
<script>
    ...
    {
        location.href = '{{ route('distributor.create') }}';
    }
    ...
    function addImagesToModal(id) 
    {
        $.ajax({
            ...
            url: '{{ route('distributor.image') }}',
            success: function(response) {
                if (response.status) {
                    $('#images_modal_body').empty();
                        $('#images_modal_body').append(`
                            <img class="img-fluid swiper_img" src="{{ asset('app-assets/images/distributor/images/${response.data}') }}" alt="'image'" />
                        `);

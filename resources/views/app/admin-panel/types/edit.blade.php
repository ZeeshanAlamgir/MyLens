...
@section('seo-breadcrumb')
    {{ Breadcrumbs::view('breadcrumbs::json-ld', 'type.create') }}
@endsection
...
@section('breadcrumbs')
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0">Update Type</h2>
                <div class="breadcrumb-wrapper">
                    {{ Breadcrumbs::render('type.edit', $type['id']) }}
                </div>
            </div>
        </div>
    </div>
...
@section('content')
    <div class="card">
        <form id="type_update__form">
            ...
            <div class="card-body">
                @csrf
                {{ view('app.admin-panel.types.form-fields',['type'=>$type]) }}
            </div>
...
<script>
    ...
    $(document).on('submit','#type_update__form',function(e){
        e.preventDefault();
        let data = new FormData(this);
        let id = {{$type->id}};
        $.ajax({
            type: "post",
            url: "{{url('admin/type/update')}}/"+id,
            data: data,
            dataType: "json",
            contentType: false,
            processData: false,
            ...
            success: function (response) {
                if(response.status==200)
                {
                    ...
                    location.href = '{{ route('type.index') }}';
                }
    ...
    FilePond.create(document.getElementById('image'), {
        ...
        files: [
            {
                source: "{{ asset('app-assets/images/type/' . $type->image) }}",
            }
        ],
    });

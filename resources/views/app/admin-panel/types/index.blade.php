...
@section('seo-breadcrumb')
    {{ Breadcrumbs::view('breadcrumbs::json-ld', 'type.index') }}
@endsection
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form id="types-datatable" action="{{ route('type.destroy')}}" method="get">
                {{ $dataTable->table() }}
            </form>
        </div>
    </div>
...
<script>
    ...
    {
        location.href = "{{ route('type.create') }}";
    }
    ...
    function addImagesToModal(id) 
    {
        $.ajax({
            ...
            url: '{{ route('type.image') }}',
            success: function(response) {
                if (response.status) {
                    $('#images_modal_body').empty();
                        $('#images_modal_body').append(`
                            <img class="img-fluid swiper_img" src="{{ asset('app-assets/images/type/${response.data}') }}" alt="'image'" />
                        `);

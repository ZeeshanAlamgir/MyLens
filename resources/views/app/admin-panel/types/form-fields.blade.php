<div class="row mb-1">
    <div class="col-lg-6 col-md-6 col-sm-6 position-relative">
        ...
        <input type="text" class="form-control form-control-lg @error('label') is-invalid @enderror" id="label"
            name="label" placeholder="Label" value="{{ isset($type) ? $type['label'] : old('label') }}" />
        @error('label')
    ...
    <input type="text" class="form-control form-control-lg @error('slug') is-invalid @enderror" placeholder="Slug" name="slug" id="slug" value="{{ isset($type) ? $type['slug'] : old('slug') }}" readonly>
    @error('slug')
        <div class="invalid-tooltip">{{ $message }}</div>
    @enderror
    </div>

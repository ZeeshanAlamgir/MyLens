    ...
    <section class="collection_section">
        <div class="container">
            ...
            <div class="row">
                ...
                {{-- @if(isset($data['search_products']) && $data['search_products']!='')
                @if(count($data['search_products'])>0) --}}
                <div class="col-md-2 col-12 pt-3" style="width: auto;">
                    {{-- <button class="btn btn-outline-primary" id="clear_filters" onclick="clearFilters()"><i
                        ...
                        {{-- <form action="{{ route('clear.filters') }}" method="GET"> --}}
                            {{-- <span class="by_online_btn ps-2 pe-2 pt-3 pb-3"  onclick="window.location.reload()" style="cursor: pointer"><i class="fa fa-times pe-1" aria-hidden="true"></i> Clear Filters</span> --}}

                            <span class="by_online_btn ps-2 pe-2 pt-3 pb-3"  id="clear_filters" style="cursor: pointer"><i class="fa fa-times pe-1" aria-hidden="true"></i> All Products</span>
                        {{-- </form> --}}
                </div>
                {{-- @endif
                @endif --}}
        ...
        @if($is_contain==true)
            <div class="container mt-5">
                <div class="row d-none empty_message_div">
                    <div class="center">
                        <center><div class="h3">OOOPPPSSS! No Product Found...</div></center>
                    </div>
                </div>
                <div class="row" id="products-main-row">
                    ...
                    @forelse ($data['collection_products'] as $collection_product)
                        <div class="col-md-2 text-center collection_color__products  col-12">
                            ...
                            <span style="font-size: 11px;">@if($collection_product['replacement_cycle']) {{ $collection_product['replacement_cycle'] ? $collection_product['replacement_cycle']['name'] : '' }} USE @endif</span>
                        </div>
                        </div>
                    @empty
            ...
            <div class="row" id="products-main-row">
                ...
                @forelse ($data['search_products'] as $search_product)
                    <div class="col-md-2 text-center collection_color__products  col-12">
                        ...
                        <span style="font-size: 11px;">@if($search_product['replacement_cycle']) {{ $search_product['replacement_cycle'] ? $search_product['replacement_cycle']['name'] : '' }} USE @endif</span>
                    </div>
                    </div>
                @empty
                    <center><h5>No Product Found</h5></center>
...
@section('custom-js')
    <script>
        ...
        $(document).on('click', '.collection_color_ul li', function() {
            ...
            // alert(collection_push__array);
            fetchFilterProducts(color_push__array, type_push__array, wear_push__array, style_push__array,collection_push__array);
        });
        ...
        function fetchFilterProducts(color_push__array, type_push__array, wear_push__array, style_push__array, collection_push__array) {
            let id = null;
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status) {
                        ...
                        if(response.collection_products.length>0)
                        {
                            $('.empty_message_div').removeClass('d-block').addClass('d-none');
                            $('#clear_filters').removeClass('d-none').addClass('d-block');
                                ...
                                <div class="col-md-2 text-center collection_color__products col-12">
                                    ...
                                    <h6 id="collection_color__products___name">${collection_product.name}</h6>
                                    <span style="font-size: 11px;">${collection_product.replacement_cycle ? collection_product.replacement_cycle.name : ''} USE</span>
                                    </div>
                                    </div>
                                `);
                            });
                        ...
                        {
                            $('.empty_message_div').removeClass('d-none').addClass('d-block');
                            $('#clear_filters').removeClass('d-block').addClass('d-none');
                        }
                    }
                },

...
namespace App\DataTables\Collection;
...
class CollectionDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query)
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->editColumn('actions', function ($row) {
                return view('app.admin-panel.collections.action', ['id' => $row->id]);
            })
            ->editColumn('banner', function ($row) {
                return view('app.admin-panel.collections.banner', ['id' => $row->id]);
            })
            ->editColumn('image', function ($row) {
                return view('app.admin-panel.collections.image', ['id' => $row->id]);
            })
            ->setRowId('id')
            ->rawColumns(array_merge($columns, ['actions', 'check','image','banner']));
    }
        ...
        return $this->builder()
            ...
            ->buttons(
                ...
                Button::raw('create-new')
                    ...
                    ->text('<i class="bi bi-plus"></i> Create New Collection')->attr([
    ...
    protected function getColumns(): array
    {
        $colArray = [
            ...
            Column::make('name')->searchable(true)->title('Color'),
            Column::make('description')->searchable(true)->title('Desc'),
            Column::make('banner')->title('Banner')->orderable(false)->searchable(false),
